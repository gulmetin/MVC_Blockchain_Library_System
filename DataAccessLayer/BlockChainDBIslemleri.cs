...
namespace DataAccessLayer
{
    public class BlockChainDBIslemleri
    {
        ...
        public void addBlock(string transaction)//veritabanında bulunan tblBlock tablosundaki bütün verileri çekiyor
        {
            string lastBlockHash = db.Database.SqlQuery<string>("SELECT TOP 1 hash FROM tblBlock ORDER BY id DESC").FirstOrDefault();
            int lastBlockId = db.Database.SqlQuery<int>("SELECT TOP 1 id FROM tblBlock ORDER BY id DESC").FirstOrDefault();
